...
import PaginationForMovieData from "./components/paginations";
...
function App() {
  ...
  const sortData = (someDataAfterDropDwn) => {
    let sorted = someDataAfterDropDwn.sort((a, b) =>
  ...
  const filterAfterSearch = () => {
    ...
    let filtered = sortData(dropDownFiltered).filter((i) =>
  ...
  return (
    <div className="container">
      <div className="row mt-5">
        <div className="mb-5">
          <Input onChange={handleChange} onKey={handleKeyDown} />
        </div>
        <div className="d-flex justify-content-between">
          <PaginationForMovieData
        ...
        {!dataLoaded ? (
          ...
          filterAfterSearch().map((star) => {
            //[20] [20-0] >>[asc] [desc]
            let knownFor = star.known_for.map(
              (i) => i.original_title || i.original_name
            ); //star.known_for:[{original_title:},{original_title},{}]
            return (
              <Card
                sx={{ maxWidth: 345 }}
                key={star.id}
              ...
              >
                ...
                <CardActions>
                  <Button size="small" onClick={() => handleModal(star.id)}>
                    show more..
                  </Button>
                </CardActions>
              </Card>
        ...
        {modalDataToBeShown && (
          <BasicModal
            ...
            openModal={openModal}
            modalDataToBeShown={modalDataToBeShown}
          />
        )}
      </div>
    </div>
